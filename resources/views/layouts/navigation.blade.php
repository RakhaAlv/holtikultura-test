...
<div x-data="{ 
    ...
    {{-- System Alerts --}}
    @if(session('success'))
        <div class="shrink-0 rounded-xl border border-emerald-200 bg-[#DCFCE7] px-4 py-2.5 text-xs font-medium text-emerald-900 shadow-sm flex items-center gap-2">
    ...
    @if(session('error'))
        <div class="shrink-0 rounded-xl border border-rose-200 bg-rose-50 px-4 py-2.5 text-xs font-medium text-rose-800 shadow-sm flex items-center gap-2">
    ...
    @if($errors->any())
        <div class="shrink-0 rounded-xl border border-rose-200 bg-rose-50 px-4 py-2.5 text-xs text-rose-800 shadow-sm">
    ...
    <div class="flex flex-1 min-h-0 flex-col justify-between overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm">
        ...
        <div class="flex-1 overflow-y-auto">
            <table class="w-full border-collapse text-left text-[13px] text-slate-700">
                <thead class="sticky top-0 z-10 border-b border-slate-100 bg-slate-50/90 backdrop-blur-sm text-xs font-bold uppercase tracking-wider text-slate-800">
                    <tr>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($users as $user)
                        <tr class="hover:bg-slate-50/80 transition">
                            <td class="px-6 py-3.5 font-medium text-slate-900">{{ $user->name }}</td>
                            <td class="px-6 py-3.5 text-slate-600">{{ $user->email }}</td>
                            <td class="px-6 py-3.5">
                                {{-- Monochromatic Green Capsule Pill Badges Sesuai Palette Khusus --}}
                                @if($user->role_id == 1)
                                ...
                                <div class="inline-flex items-center gap-2">
                                    ...
                                    <button 
                                        @click="
                                            ...
                                            openEdit = true;
                                        " 
                                        class="inline-flex items-center rounded-lg border border-[#16B33A] bg-emerald-50/60 px-3 py-1 text-xs font-semibold text-[#16B33A] hover:bg-[#16B33A] hover:text-white transition">
                                        Edit
                                    </button>
                                    ...
                                    @if(auth()->id() !== $user->getKey())
                                        <form action="{{ route('users.destroy', $user->getKey()) }}" method="POST" onsubmit="return confirm('Hapus pengguna ini secara permanen?')" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="inline-flex items-center rounded-lg border border-rose-300 bg-rose-50/50 px-3 py-1 text-xs font-semibold text-rose-600 hover:bg-rose-600 hover:text-white transition">
                                                Hapus
                                            </button>
                                        </form>
                                    @endif
